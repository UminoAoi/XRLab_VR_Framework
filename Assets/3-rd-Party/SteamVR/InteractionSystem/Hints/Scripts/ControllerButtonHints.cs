...
namespace Assets.SteamVR.InteractionSystem.Hints.Scripts
{
	...
	public class ControllerButtonHints : MonoBehaviour
	{
		...
		private void HideAllButtonHints()
		{
            ...
            if (renderModel != null && renderModel.gameObject != null && hand.hintsActive == 0) {
  ...
  //                  if (action.GetActive(inputSource))
  //                  {
  //                      ShowText(action, action.GetShortName(), true);
  ...
  //              }
		...
		{
			renderers.Clear();
            hand.hintsActive -= flashingRenderers.Count;
            flashingRenderers.Clear();
		...
		private void HideAllText()
		{
            if (actionHintInfos != null)
            {
                ...
                foreach (var hintInfo in actionHintInfos) {
                    if (hand != null && hintInfo.Value.textHintActive) {
                        hand.hintsActive--;
                        if (!hand.isControllerActive && hand.hintsActive == 0) {
                            if (!hand.isControllerActive)
                                hand.ResetTemporarySkeletonRangeOfMotion();
                            renderModel.gameObject.SetActive(false);
                        }
